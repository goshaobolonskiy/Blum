...
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJoYXNfZ3Vlc3QiOmZhbHNlLCJ0eXBlIjoiQUNDRVNTIiwiaXNzIjoiYmx1bSIsInN1YiI6ImI3Mzg4YTAwLTRhZWEtNDA3MS05YjA2LThjYzNiNDIyMDczYSIsImV4cCI6MTczNjAxNDY1OCwiaWF0IjoxNzM2MDExMDU4fQ.YC7lDM85TkE0bkejN6803SRSyWwJIhiyNaxrsnbyqy0"
...
def claim_task(task_id):
        with open('data.txt', 'a', encoding='utf-8') as file:
            ...
            if response.status_code != 200:
                ...
                file.write(f'Task with id = {task_id} finished')
...
def start_game():
    ...
    with open('data.txt', 'a', encoding='utf-8') as file:
        ...
        if playPasses <= 0:
            ...
            file.write("Not enough tikets to start play game")
            return
        ...
        while playPasses >= 0:
            ...
            if response.ok:
                ...
                file.write(f"Started playing game. Sleep {int(sleep_time)}s...")
                ...
                if response.status_code != 200:
                    file.write("Can not found game - is not claim")
                    print("Can not found game")
                    
                if not response.ok:
                    file.write(f"Can not claim game with status code {response.status_code}: {response.text}")
                ...
                if response.ok:
                    ...
                    file.write(f"Claimed game, earned: {blum_amount}BP")
                ...
                file.write(f"Can not start play with status code {response.status_code}: {response.text}")
        ...
        try:
            ...
            if status == 200:                                                   # Если запрос удачный, то продолжаем
                ...
                if "farming" in balance:                                        # Если фарминг был запущен
                    ...
                    farm = "is not farming"
                    go_farming()                                                          # Если фарминг не был запущен, то
                    time.sleep(3)                                                                 # запускаем
                    get_balance()
                ...
                print(f'When trying to find out the balance, an error occurred {status}')
        except ValueError as e:
            print(f"Error: {e}")
